...
def rename_cols(dataframe:pd.DataFrame)->pd.DataFrame:
    new_df = dataframe.copy()
    new_cols_name = pd.Series(dataframe.columns).str.strip()\
                                     .str.lower()\
...
def clear_numeric_strings(dataframe:pd.DataFrame,subset=Union[str,list])->pd.DataFrame:
    new_df = dataframe.copy()
...
def convert_to_numeric(dataframe:pd.DataFrame,subset:Union[str,list])->pd.DataFrame:
    new_df = dataframe.copy()
    new_df[subset]=new_df[subset].astype("float")
    return new_df


def add_to_pipe(dataframe:pd.DataFrame,func=callable,*args,**kwargs)-> pd.DataFrame.pipe:
...
def make_pipeline(dataframe:pd.DataFrame,functions:list)->pd.DataFrame:
    ...
    for f in functions:
        if "function_args" in f.keys() and "function_kwargs" in f.keys():
            try:
                new_df = add_to_pipe(new_df,*f["function_args"],func=f["function"],**f["function_kwargs"])
            except TypeError:
                print("function_args must be a iterable and function_kwargs must be a dict.")
...
list_funcs = [ 
    {
        "function":drop_cols,
        "function_kwargs":{"subset":["Emite boletos.1","ID"]}
    },
    {
        "function":rename_cols,
    },
    {
        "function":clear_numeric_strings,
        "function_kwargs":{"subset":NUM_FEATS}
    },
    {
